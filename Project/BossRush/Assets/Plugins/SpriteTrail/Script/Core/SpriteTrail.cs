...
using UnityEngine.Serialization;
    ...
    public sealed class SpriteTrail: MonoBehaviour {
		...
		private Quaternion m_PreviousRotSpawned;
        ...
        public void EnableTrail() {
			...
			m_PreviousRotSpawned = m_SpriteToDuplicate.transform.rotation;
                ...
                switch (m_CurrentTrailPreset.m_TrailElementSpawnCondition) {
                    ...
                    case TrailElementSpawnCondition.Distance:
                        if (m_CurrentTrailPreset.m_DistanceCorrection && m_CurrentTrailPreset.m_DistanceBetweenSpawns > 0) {
							float distSincePrevious = Vector2.Distance(m_PreviousPosSpawned, m_SpriteToDuplicate.transform.position);
							int iterations = (int)(distSincePrevious / m_CurrentTrailPreset.m_DistanceBetweenSpawns);

							//TODO : if error corrected : generate a timer error ( 3 elements generated at the same frame == 3 elements that update the same)
							if(iterations > 0)
							{
								int curIteration = 1;
								Quaternion startRot = m_PreviousRotSpawned;
								...
								{
									curRot = Quaternion.Lerp(m_PreviousRotSpawned, m_SpriteToDuplicate.transform.rotation, (1f/ (float)iterations) * curIteration);
									...
									GenerateNewTrailElement(new Vector3(_tmpPos.x, _tmpPos.y, m_SpriteToDuplicate.transform.position.z), true, curRot);
									m_PreviousPosSpawned = _tmpPos;
									m_PreviousRotSpawned = curRot;
									curIteration++;
								}
							}
		...
		void GenerateNewTrailElement(Vector3 pos, bool useCustomRotation = false, Quaternion overrideRot = new Quaternion() ) {
			...
			if (useCustomRotation)
				_TmpElement.m_Transform.rotation = overrideRot;
