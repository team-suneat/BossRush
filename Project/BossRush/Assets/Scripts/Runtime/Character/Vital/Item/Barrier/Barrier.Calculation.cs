...
{
    /// <summary> Barrier 클래스의 계산 관련 기능을 담당합니다. </summary>
    public partial class Barrier
    {
        ...
        public void RefreshMaxValue(bool shouldAddExcessToCurrent = false, bool shouldLoadCurrentValueToMax = false)
        {
            ...
            int previousMax = Max;
            BarrierCalculationData calculationData = CalculateBarrierValues();
        ...
        private BarrierCalculationData CalculateBarrierValues()
        {
            float multiplier = FindStatValueByOwner(StatNames.BarrierMulti);
            float fixedValue = FindStatValueByOwner(StatNames.Barrier);

            return new BarrierCalculationData
            {
        ...
        private void SetNewMaxValue(BarrierCalculationData data)
        {
